(() => {
  ...
  class CustomAudioPlayer {
    ...
    applyFloatingLyricVisibility() {
      const shouldShow = this.hasActiveLyric && this.isFloatingLyricVisible;
      this.elements.floatingLyric.hidden = !shouldShow;
      if (!shouldShow) {
    ...
    updateLyricSwitch() {
      const disabled = this.isLyricLoading || !this.hasActiveLyric;
      if (this.isLyricLoading) {
        this.elements.lyricSwitch.textContent = "加载中";
        this.elements.lyricSwitch.title = "正在加载歌词";
      } else if (!this.hasActiveLyric) {
        this.elements.lyricSwitch.textContent = "无歌词";
        this.elements.lyricSwitch.title = "当前歌曲无歌词";
      } else {
      ...
      this.elements.lyricSwitch.disabled = disabled;
      this.elements.lyricSwitch.setAttribute("aria-disabled", disabled ? "true" : "false");
      this.elements.lyricSwitch.setAttribute(
        "aria-pressed",
        !disabled && this.isFloatingLyricVisible ? "true" : "false"
      );
    }
    ...
    async loadLyric(track) {
      ...
      this.lyricCursor = -1;
      this.hasActiveLyric = false;
      this.isLyricLoading = Boolean(track.lrc);

      const token = this.lyricToken + 1;
      this.lyricToken = token;

      this.setFloatingLyric(track.lrc ? "正在加载歌词..." : "暂无歌词", "");
      this.applyFloatingLyricVisibility();
    ...
    startFloatingLyricDrag(event) {
      if (!this.hasActiveLyric || !this.isFloatingLyricVisible || !event.isPrimary) {
        return;
      }
    ...
    ensureFloatingLyricInViewport() {
      if (!this.hasActiveLyric || !this.isFloatingLyricVisible) {
        return;
      }
      const lyric = this.elements.floatingLyric;
    ...
    syncLyric(force) {
      ...
      if (match === -1) {
        this.setFloatingLyric(
          this.lyrics[0] ? this.lyrics[0].text : " ",
          this.lyrics[1] ? this.lyrics[1].text : ""
        );
        return;
      }
